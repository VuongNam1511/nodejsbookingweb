...
import _, { includes, iteratee } from 'lodash'
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (checkObj.isValid === false) {
                resolve({
                    errCode: 1,
                    errMessage: `Missing requered Parameter: ${checkObj.element}`
                })
            } else {
                //upsert to markdown 
    ...
    return new Promise(async (resolve, reject) => {
        try {
            if (!data.arrSchedule || !data.doctorId || !data.formatedDate) {
                resolve({
                    errCode: 1,
                    errMessage: 'Missing requered Paramter !'
                })
            } else {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            if (!idInput) {
                ...
                let data = await db.Doctor_Infor.findOne({ //findOne trả ra 1 object còn findAll trả ra 1 array
                    where: {
                        doctorId: idInput
                    },
                    attributes: {
